...
-- ============================================================================
-- NOTE: Cron job creation requires superuser privileges or specific permissions.
-- If this fails, you can create the cron job manually via Supabase Dashboard
-- or run the SQL below with elevated privileges.
...
DECLARE
  ...
  has_cron_permission BOOLEAN := false;
BEGIN
  -- Check if we have permission to access cron.job table
  BEGIN
    SELECT EXISTS (
    ...
    has_cron_permission := true;
  EXCEPTION WHEN insufficient_privilege OR OTHERS THEN
    RAISE NOTICE 'Cannot access cron.job table. Cron job must be created manually.';
    RAISE NOTICE 'See create-validation-worker-cron-job.sql for manual creation instructions.';
    RETURN;
  END;
  
  -- If we have permission, proceed with cron job creation
  IF has_cron_permission THEN
    ...
    IF job_id IS NULL THEN
      ...
      EXCEPTION WHEN insufficient_privilege OR OTHERS THEN
        RAISE NOTICE 'Failed to create cron job: %. Please create manually.', SQLERRM;
        RAISE NOTICE 'See Supabase Dashboard > Database > Cron Jobs to create manually.';
      END;
    ELSE
